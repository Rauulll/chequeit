defmodule ChequeitWeb.UserSettingsControllerTest do
  ...
  describe "PUT /auth/settings (change password form)" do
    test "updates the user password and resets tokens", %{conn: conn, user: user} do
      ...
      assert redirected_to(new_password_conn) == ~p"/auth/settings"
  ...
  describe "PUT /auth/settings (change email form)" do
    ...
    test "updates the user email", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == ~p"/auth/settings"
  ...
  describe "GET /auth/settings/confirm_email/:token" do
    setup %{user: user} do
    ...
    test "does not update email with invalid token", %{conn: conn, user: user} do
      conn = get(conn, ~p"/auth/settings/confirm_email/oops")
      assert redirected_to(conn) == ~p"/auth/settings"
